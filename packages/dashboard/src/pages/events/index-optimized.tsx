...
import { useEventsOptimized } from "../../lib/hooks/events-optimized";
...
export default function Index() {
	...
	const { data: events, mutate } = useEventsOptimized();
	...
	const getChartData = (triggersByMonth: Record<string, number>) => {
		return Object.entries(triggersByMonth)
			...
			<Dashboard>
				...
				>
					{events && contacts ? (
						regularEvents.length > 0 ? (
							<Table
								values={regularEvents.map((e) => {
									const percentage = contacts > 0 
										? Math.round((e.uniqueContactCount / contacts) * 100)
										: 0;

									return {
										Event: e.name,
										"Triggered by users": (
											<Badge type={"info"}>{`${percentage}%`}</Badge>
										),
										"Total triggers": e.totalTriggers,
										Timeline: (
											<>
												<ResponsiveContainer width={100} height={40}>
													<AreaChart
														width={100}
														height={40}
														data={getChartData(e.triggersByMonth)}
														margin={{
															top: 5,
															right: 0,
															left: 0,
										...
										"Last Activity": dayjs().to(e.lastActivity).toString(),
										Trigger: (
											<button
												onClick={() => {
													toast.promise(
							...
							<Table
								values={templateEvents.map((e) => {
									...
									return {
										...
										"Total times triggered": e.totalTriggers,
										"Last Activity": dayjs().to(e.lastActivity).toString(),
									};
								})}
							/>
